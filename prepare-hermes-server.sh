#!/usr/bin/env bash
# ============================================================================
# prepare-hermes-server.sh
# ----------------------------------------------------------------------------
# Готовит свежий сервер (Ubuntu 22.04 / 24.04) к установке Hermes Agent:
#   1. Обновляет систему (неинтерактивно, без диалогов про конфиги)
#   2. Ставит все обязательные и рекомендуемые зависимости
#   3. Создаёт отдельного пользователя `hermes` с автогенерированным паролем
#   4. Включает linger — чтобы агент работал даже когда ты не в SSH
#
# После этого скрипта останется одна интерактивная часть — пройти wizard
# Hermes под пользователем hermes. Инструкции скрипт распечатает в конце.
#
# Использование:
#   curl -fsSL https://<твой-хост>/prepare-hermes-server.sh | bash
#
# Или, чтобы сначала прочитать скрипт:
#   curl -fsSL https://<твой-хост>/prepare-hermes-server.sh | less
# ============================================================================
...
# --- Финал — инструкции для пользователя ------------------------------------
cat <<EOF
...
${GREEN}${BOLD}════════════════════════════════════════════════════════════════════${RESET}
  ...
  ${BOLD}СОХРАНИ ЭТОТ ПАРОЛЬ${RESET} — он понадобится чтобы переключаться
  на пользователя hermes. Скопируй его в менеджер паролей прямо сейчас.

${GREEN}${BOLD}════════════════════════════════════════════════════════════════════${RESET}

  ${BOLD}Что дальше:${RESET}

  ${BLUE}1.${RESET} Переключись на пользователя hermes (попросит пароль выше):

       ${BOLD}su - hermes${RESET}

  ${BLUE}2.${RESET} Запусти официальный установщик Hermes:

       ${BOLD}curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash${RESET}

  ${BLUE}3.${RESET} Пройди wizard — выбирай эти варианты:

       • How would you like to set up Hermes?     → ${BOLD}Quick setup${RESET}
       • Select provider:                          → ${BOLD}Leave unchanged${RESET}
       • Select terminal backend:                  → ${BOLD}Keep current (local)${RESET}
       • Connect a messaging platform?             → ${BOLD}Skip${RESET}
       • Install gateway as systemd service? [Y/n] → ${BOLD}Y${RESET}
       • Choose how the gateway should run:        → ${BOLD}User service${RESET}

  ${BLUE}4.${RESET} Активируй команду hermes в текущей сессии:

       ${BOLD}source ~/.bashrc${RESET}

  ${BLUE}5.${RESET} Hermes установлен! Дальнейшие шаги — в главе 2.2 гайда
     (подключение модели через web UI).
